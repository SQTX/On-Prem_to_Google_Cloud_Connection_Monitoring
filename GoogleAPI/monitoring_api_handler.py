...
class Monitoring:
    ...
    def write_time_series(self, value, metric_type_name, metric_store_id):
        ...
        if series == None: series = self.setup_metric(metric_type_name, metric_store_id)
        series = self.add_point_to_series(value, series)

        self.monitoring_client.create_time_series(request={"name": f'projects/{PROJECT_ID}', "time_series": [series]})